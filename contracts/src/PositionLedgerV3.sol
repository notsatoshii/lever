...
/**
 * @title PositionLedgerV3
 * @author LEVER Protocol
 * @notice Core position tracking with lazy fee accrual for synthetic perpetuals
 * @dev V3 Changes:
 *      - Fixed double OI subtraction bug in close flow
 *      - _updateOI now handles all OI changes including totalGlobalOI
 *      - _closePosition no longer modifies OI (already done by _updateOI)
 */
contract PositionLedgerV3 {
    ...
    /**
     * @notice Migrate position from old ledger (owner only, one-time migration)
     * @dev Used to migrate positions from V2 to V3 without re-opening
     */
    function migratePosition(
        ...
        require(pos.size == 0, "Position exists");
        ...
        if (size > 0) {
            market.totalLongOI += absSize;
        } else if (size < 0) {
            market.totalShortOI += absSize;
        }
        totalGlobalOI += absSize;
        
        emit PositionOpened(trader, marketId, size, entryPrice, collateral);
    }
    ...
    function openPosition(
        ...
        // Update OI (handles global OI tracking internally)
        _updateOI(market, pos.size, sizeDelta);
        
        // Check global OI cap (only matters when increasing)
    ...
    function _closePosition(address trader, uint256 marketId, uint256 exitPrice) internal {
        ...
        // NOTE: OI is already updated by _updateOI() which is called before this
        // Do NOT subtract here to avoid double-subtraction bug
    ...
    function _updateOI(Market storage market, int256 oldSize, int256 sizeDelta) internal {
        ...
        // Update global OI (subtract old, add new)
        if (oldSize != 0) {
            totalGlobalOI -= _abs(oldSize);
        }
        ...
        // If newSize == 0, no new OI to add (position fully closed)
    }
