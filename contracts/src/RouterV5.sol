 ...
 * @notice Main entry point with proper LP Pool integration
 * @dev Changes from V4:
 *      - Calls lpPool.allocate() on position open
 *      - Calls lpPool.deallocate() on position close
 *      - Routes borrow/trading fees to LP pool via addFees()
 *      - Uses smoothed PI from PriceEngineV2 for mark price
 */
...
contract RouterV5 {
    ...
    address public priceEngine;         // Mark price (PriceEngineV2 - smoothed PI)
    address public positionLedger;      // Position state
    ...
    event FeesRouted(uint256 amount, address indexed lpPool);
    event LPAllocated(uint256 amount, uint256 totalAllocated);
    ...
    function openPosition(
        ...
        // Get Mark Price (smoothed PI) from PriceEngineV2 for risk checks
        ...
        // *** LP INTEGRATION: Allocate capital ***
    ...
    function closePosition(
        ...
        // Calculate PnL using exit price
        if (size > 0) {
        ...
        if (pnl >= 0) {
            ...
            if (loss + feesToCollect >= collateralToReturn) {
                amountOut = 0;
            } else {
                amountOut = collateralToReturn - loss - feesToCollect;
            }
        }
        ...
        // *** LP INTEGRATION: Deallocate capital ***
        _deallocateFromLP(closeSize);
        
        // *** LP INTEGRATION: Route fees to LP pool ***
        if (feesToCollect > 0) {
        ...
        emit PositionClosed(msg.sender, marketId, closeSize, exitPrice, pnl, feesToCollect);
    }
    ...
    // ============ LP Integration Functions ============
    
    /**
     * @notice Allocate capital from LP pool for new position
     */
    function _allocateToLP(uint256 amount) internal {
        if (lpPool == address(0)) return;
        
        try ILPPool(lpPool).allocate(amount) {
            emit LPAllocated(amount, 0);
        } catch {
            // LP allocation failed - log but don't revert
            // This allows trading to continue even if LP tracking has issues
        }
    }
    
    /**
     * @notice Deallocate capital back to LP pool when position closes
     */
    function _deallocateFromLP(uint256 amount) internal {
        if (lpPool == address(0)) return;
        
        try ILPPool(lpPool).deallocate(amount) {
            emit LPDeallocated(amount);
        } catch {
            // LP deallocation failed - log but don't revert
        }
    }
    ...
    function getPositionDetails(
        ...
        // Calculate unrealized PnL using Mark Price (smoothed PI)
    ...
    function _getMarkPrice(uint256 marketId) internal view returns (uint256) {
        // Get smoothed PI from PriceEngineV2
